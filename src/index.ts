...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Propublica Nonprofit');
}
...
async function searchNonprofits(query: string, state?: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/search.json?${params}`);
  if (!res.ok) throw await httpError(res, 'ProPublica Nonprofit API error');
...
async function getOrganization(ein: number) {
  const res = await pwFetch(`${BASE_URL}/organizations/${ein}.json`);
