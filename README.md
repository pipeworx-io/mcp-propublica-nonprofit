...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_nonprofits \
  -H 'Content-Type: application/json' \
  -d '{"query":"red cross"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_nonprofits`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "propublica-nonprofit": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-propublica-nonprofit"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-propublica-nonprofit
```
